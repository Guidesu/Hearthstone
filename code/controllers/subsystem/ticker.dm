...
SUBSYSTEM_DEF(ticker)
	...
	var/rulertype = "Leader" // reports whether leader or queen rules
...
/datum/controller/subsystem/ticker/proc/checkreqroles()
	...
	//var/list/required_jobs = list("Path Maiden","Leader","Merchant") //JTGSZ - 4/11/2024 - This was the prev set of required jobs to go with the hardcoded checks commented out below
	...
	for(var/V in required_jobs)
		for(var/mob/dead/new_player/player in GLOB.player_list)
			...
			if(player.client.prefs.job_preferences[V] == JP_HIGH)
				...
				readied_jobs.Add(V)
		/*
			// These else conditions stop the round from starting unless there is a merchant, leader, and queen.
		else
