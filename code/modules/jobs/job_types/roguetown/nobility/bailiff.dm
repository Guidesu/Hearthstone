/datum/job/roguetown/bailiff
	...
	tutorial = "You judge the common folk and their wrongdoings if necessary. You help plan with the Councillors and maybe the Leader on any new issues, laws, judgings, and construction that are required to adapt to the world. You have two assistant Councillors that may serve as jurors to assist you in your job. You are required to enforce taxes for the Leader, judge people for breaking the law, make sure the town and manor are not in decay, and to help plan or construct new buildings. You are allowed some limited control over Guards, however it is not the focus of your job unless special circumstances are to change this."
	whitelist_req = FALSE
...
/proc/find_lord(required_stat = CONSCIOUS)
	...
	for(var/mob/living/carbon/human/H in GLOB.human_list)
		if(!H.mind || H.job != "Leader" || (H.stat > required_stat))
			continue
		lord = H
		break
	return lord
