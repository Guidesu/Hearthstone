...
/obj/structure/roguemachine/titan/proc/give_tax_popup(mob/living/carbon/human/user)
	...
	if(newtax)
		...
		priority_announce("The new tax in Emberstead shall be [newtax] percent.", "The Generous Lord Decrees", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Captain")
...
/obj/structure/roguemachine/titan/proc/declare_outlaw(mob/living/user, raw_message)
	if(!SScommunications.can_announce(user))
		return
	if(user.job)
		if(!istype(SSjob.GetJob(user.job), /datum/job/roguetown/priest/lord))
			return
	else
		return
...
/proc/make_outlaw(raw_message)
	if(raw_message in GLOB.outlawed_players)
		...
		priority_announce("[raw_message] is no longer an outlaw in Emberstead lands.", "The Leader Decrees", 'sound/misc/royal_decree.ogg', "Captain")
		return FALSE
	var/found = FALSE
	...
	if(!found)
		return FALSE
	GLOB.outlawed_players += raw_message
	priority_announce("[raw_message] has been declared an outlaw and must be captured or slain.", "The Leader Decrees", 'sound/misc/royal_decree2.ogg', "Captain")
	return TRUE
