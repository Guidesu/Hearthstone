	...
	if(ttime >= GLOB.round_timer)
		if(roundvoteend)
			...
			if(!SSvote.mode && SSticker.autovote)
				SSvote.initiate_vote("endround", pick("Zlod", "Sun Leader", "Gaia", "Moon Queen", "Aeon", "Gemini", "Aries"))
...
/datum/game_mode/chaosmode/proc/pick_bandits()
	...
	if(num_bandits)
		...
		/*
			...
			so if it was working a medium priority leader would still get shunted into a bandit.
...
/datum/game_mode/chaosmode/proc/pick_maniac()
	restricted_jobs = list("Leader", "Queen Consort")
	...
	for(var/datum/mind/rogue in pre_aspirants)
		switch(rogue.special_role)
			...
			if("Supporter")
				...
				aspirants += rogue
				pre_aspirants -= rogue
	var/mob/living/leader = SSticker.rulermob
	if(leader)
		var/datum/antagonist/ruler = new /datum/antagonist/aspirant/ruler() // Do the leader last.
		leader.mind.add_antag_datum(ruler)
